...
def vqa_collate_fn(batch, processor, device):
...
def eval_loop(model, eval_loader, processor, epoch):
    model.eval()
    preds, refs = [], []
    ...
    for batch in eval_bar:
        with torch.no_grad():
            ...
            preds.extend(processor.tokenizer.batch_decode(gen_ids, skip_special_tokens=True))
            refs.extend(processor.tokenizer.batch_decode(batch['labels'], skip_special_tokens=True))
...
def main():
    ...
    train_ds = create_dataset(train_df, IMAGE_DIR)
    val_ds = create_dataset(val_df, IMAGE_DIR)
    print(f"train={len(train_ds)}, val={len(val_ds)}")

    train_loader = DataLoader(
        train_ds, batch_size=BATCH_SIZE, shuffle=True,
        collate_fn=lambda batch: vqa_collate_fn(batch, processor, device)
    )
